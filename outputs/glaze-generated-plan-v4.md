...
| Account | `delete_my_data` | **blocks if caller is a sole owner** (must transfer/dissolve first); removes presence, signals, resource metadata, and pending requests; deletes/tombstones the caller's own messages; **pseudonymizes retained shared ledger rows** (Amdt 4) rather than deleting them |
...
**Deletion semantics (v3 correction — reconciling immutable ledger with the delete promise):**
...
- **Own messages:** the caller's `session_messages` are deleted or tombstoned ("message deleted"); the **counterparty's** messages remain (they are that user's data).
- **Shared ledger entries are NOT deleted** — they are a two-party transparency record the other member relies on. Instead the departing user is **pseudonymized**: their `user_id` is replaced with a stable tombstone pseudonym, leaving accurate accounting (times, usage, decision, outcome) with **no personal identifier and never any secret**. The counterparty keeps a truthful record; the deleted user is de-identified.
...
**New / changed tables:** add `invitations` (non-secret `id`, `secret_hash` via pgcrypto bcrypt, community_id, expiry, max_uses, used_count, revoked_at) and `blocks` (blocker_id, blocked_id, created_at, unique pair). `memberships` gains `status`. `ledger_entries` supports a **pseudonymized tombstone** for a deleted party (nullable/replaceable actor identifier, still secret-free and immutable). `resource_requests` carries the bounded job envelope + result reference + usage. Requires the `pgcrypto` extension. Full set (RLS on all): `users`, `communities`, `memberships`, `invitations`, `blocks`, `signals`, `presence`, `handshakes`, `shared_sessions`, `session_messages`, `resources` (non-secret metadata only), `resource_requests`, `ledger_entries`.
...
- **Deletion consistency:** `delete_my_data` removes personal rows and tombstones own messages while the counterparty's messages persist; **ledger rows are retained but pseudonymized** (no personal id, no secret) and remain UPDATE/DELETE-blocked by the immutability trigger.
