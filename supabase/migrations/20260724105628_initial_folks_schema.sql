...
create table if not exists public.shared_sessions (
  id uuid primary key default gen_random_uuid(),
  handshake_id uuid not null unique references public.handshakes (id) on delete cascade,
  community_id uuid not null references public.communities (id) on delete cascade,
...
declare
  ...
  folks_tables text[] := array[
    ...
    'public.ledger_entries','folks_private.invitation_throttle'
  ];
...
declare
  ...
  rt_tables text[] := array[
    ...
    'resource_requests','ledger_entries'
  ];
begin
